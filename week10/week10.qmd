---
title: "GLM_binaryoutcomes_Week10"
author: "Michael Cao"
date: 3/24/25
format:
  html:
...
We can write probability as an function of odds where: 

$odds_{biden_{i} = 1} = \frac{P(biden_{i} = 1)}{1 - P(biden_{i} = 1)}$

Taking the log odds, we can rewrite our model equation as: 

$P(biden_{i} = 1) = \frac{e^{\alpha + \beta(deathpen_{i})}}{1 + e^{\alpha + \beta(deathpen_{i})}}$

Where $\beta$ essentially becomes the log-difference of odds, which is equivalent to the log odds ratio: 

$\beta = log(OR_{biden_{i}} = log\frac{P(biden_{i} = 1) | P(deathpen_{i} = 1)}{1 - P(biden_{i} = 1) | P(deathpen_{i} = 1)} - log\frac{P(biden_{i} = 1) | P(deathpen_{i} = 0)}{1 - P(biden_{i} = 1) | P(deathpen_{i} = 0)}$

```{r, echo = TRUE}
model3 <- glm(biden ~ deathpen,
              data = data1,
              family = binomial(link = "logit"))
#We change the link to logit in order to take the log odds of the left-hand side and
#keep the right-side linear. 
broom::tidy(model3,
            conf.int = TRUE,
            conf.level = 0.99)
...
We can interpret the results of this odds ratio regression to mean that if we exponentiate the logged $\beta$ or logged odds differences in our groups, we can say someone supports capital punishment has much lower odds (0.19 times) of supporting Biden than the odds of someone who does not support capital punishment. The 99% confidence interval for this odds ratio would be $\{0.154, 0.251\}$.
...
```{r, echo = TRUE}
model5 <- glm(havekids ~ evermarried,
              data = data2,
              family = binomial(link = "log"))
...
$odds_{havekids_{i} = 1} = \frac{P(havekids_{i} = 1)}{1 - P(havekids_{i} = 1)}$

Taking the log odds, we can rewrite our model equation as: 

$P(havekids_{i} = 1) = \frac{e^{\alpha + \beta(evermarried_{i})}}{1 + e^{\alpha + \beta(evermarried_{i})}}$

Where $\beta$ essentially becomes the log-difference of odds, which is equivalent to the log odds ratio: 

$\beta = log(OR_{havekids_{i}} = log\frac{P(havekids_{i} = 1) | P(evermarried_{i} = 1)}{1 - P(havekids_{i} = 1) | P(evermarried_{i} = 1)} - log\frac{P(havekids_{i} = 1) | P(evermarried_{i} = 0)}{1 - P(havekids_{i} = 1) | P(evermarried_{i} = 0)}$

```{r, echo = TRUE}
model6 <- glm(havekids ~ evermarried,
              data = data2,
              family = binomial(link = "logit"))
#We change the link to logit in order to take the log odds of the left-hand side and
#keep the right-side linear. 
...
We can interpret the results of this odds ratio regression to mean that if we exponentiate the logged $\beta$ or logged odds differences in our groups, we can say someone is ever-married has much higher odds (10.6 times) of having kids than the odds someone who has never been married. The 99% confidence interval for this odds ratio would be $\{8.67, 13.1\}$.
